...
export function LineChart({
  ...
  const { path, area, points, yTicks, xs } = useMemo(() => {
    if (!data.length)
      return {
        path: "",
        area: "",
        points: [] as { cx: number; cy: number; p: Point }[],
        yTicks: [] as number[],
        xs: [] as { x: number; label: string }[],
      };
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} className="w-full">
      ...
      <text x={6} y={padT - 2} fontSize="8" fill="currentColor" opacity="0.5">
        {yLabel}
      </text>
      {data.length > 0 && (
        <>
          ...
          {points.map((pt, i) => (
            <motion.circle
              ...
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              transition={{ delay: 0.5 + i * 0.05, type: "spring", stiffness: 240 }}
            />
          ))}
          {xs
            .filter((_, i) => i % Math.max(1, Math.ceil(xs.length / 5)) === 0)
            .map((t, i) => (
              <text
                key={i}
                x={t.x}
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} className="w-full">
        ...
        return (
          <g key={i}>
            <line
              ...
              y2={y}
              stroke="currentColor"
              strokeOpacity={i === 0 || i === 4 ? 0.18 : 0.07}
            />
          </g>
        );
      })}
      ...
      {normSeries.map((s, si) => {
        ...
        const area =
          pts.length > 1
            ? `${path} L${pts[pts.length - 1].cx},${padT + innerH} L${pts[0].cx},${padT + innerH} Z`
            : "";
          ...
          <g key={si}>
            ...
            {pts.map((pt, i) => (
              <motion.circle
                ...
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ delay: 0.4 + i * 0.04 + si * 0.1, type: "spring", stiffness: 240 }}
              />
            ))}
...
export function BarChart({
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} className="w-full">
      ...
      {yLabel && (
        <text x={6} y={padT - 3} fontSize="8" fill="currentColor" opacity="0.55">
          {yLabel}
        </text>
      )}
      {data.map((d, i) => {
      ...
      {zones.map((z, i) => {
        ...
        return (
          <text
            ...
            style={active ? { filter: `drop-shadow(0 0 4px ${z.c})` } : undefined}
          >
            {z.label}
          </text>
        );
...
export function StackedCompareBar({
  rows,
}: {
  rows: { label: string; value: number; target: number; color: string }[];
}) {
  return (
          ...
          <div key={i}>
            <div className="flex items-center justify-between text-xs mb-1">
              ...
              <span className="font-bold">
                {r.value} <span className="text-muted-foreground font-medium">/ {r.target}</span>
              </span>
            </div>
...
export function RadarChart({
  ...
  const polyPts = values
    .map((v, i) => pt(i, Math.min(1, Math.max(0, v))))
    .map((p) => `${p.x},${p.y}`)
    .join(" ");
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full max-w-[280px]">
      {grid.map((g, i) => {
        const pts = Array.from({ length: n }, (_, k) => pt(k, g))
          .map((p) => `${p.x},${p.y}`)
          .join(" ");
        return (
          <polygon key={i} points={pts} fill="none" stroke="currentColor" strokeOpacity={0.1} />
        );
      })}
      {axes.map((_, i) => {
        const p = pt(i, 1);
