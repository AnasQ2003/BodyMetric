...
const themes: Record<string, { a: string; b: string; c: string; tint: string; ring: string }> = {
  under: {
    a: "oklch(0.78 0.16 220)",
    b: "oklch(0.78 0.14 200)",
    c: "oklch(0.85 0.12 250)",
    tint: "oklch(0.96 0.04 220)",
    ring: "oklch(0.62 0.22 220)",
  },
  healthy: {
    a: "oklch(0.78 0.18 155)",
    b: "oklch(0.8 0.14 180)",
    c: "oklch(0.85 0.16 130)",
    tint: "oklch(0.96 0.04 155)",
    ring: "oklch(0.6 0.18 155)",
  },
  over: {
    a: "oklch(0.82 0.17 75)",
    b: "oklch(0.78 0.18 40)",
    c: "oklch(0.85 0.14 95)",
    tint: "oklch(0.96 0.04 75)",
    ring: "oklch(0.65 0.2 60)",
  },
  obese: {
    a: "oklch(0.7 0.22 25)",
    b: "oklch(0.7 0.22 350)",
    c: "oklch(0.75 0.2 10)",
...
export function AppFrame({ children }: { children: ReactNode }) {
  ...
  const cat = useMemo(
    () => bmiCategory(calcBMI(profile.weight, profile.height)),
    ...
    >
      ...
      <div className="mx-auto grid h-full w-full max-w-[calc(100%-32px)] grid-rows-[auto_1fr_auto] relative">
        <header className="z-30 px-4 pt-6">
          <motion.div
            layout
            className="glass relative flex items-center justify-between rounded-2xl px-3 py-2.5 shadow-glow ring-1 ring-white/40 overflow-hidden"
          >
            <span
              aria-hidden
              className="pointer-events-none absolute inset-0 rounded-2xl"
              style={{
                boxShadow: `inset 0 1px 0 rgba(255,255,255,0.6), inset 0 0 24px color-mix(in oklab, ${theme.ring} 32%, transparent)`,
              }}
            />
            <span
              aria-hidden
              ...
              style={{
                background: `linear-gradient(120deg, transparent 30%, color-mix(in oklab, ${theme.a} 55%, transparent) 50%, transparent 70%)`,
        ...
        <nav className="relative z-30 px-3 pb-4 pt-1">
          <motion.span
            aria-hidden
            className="pointer-events-none absolute inset-x-2 bottom-2 top-1 rounded-[28px] blur-xl opacity-60"
          ...
          <div className="glass relative flex items-center justify-between rounded-3xl p-2 shadow-glow ring-1 ring-white/40 overflow-hidden">
            <span
              ...
              style={{
                boxShadow: `inset 0 1px 0 rgba(255,255,255,0.6), inset 0 0 28px color-mix(in oklab, ${theme.ring} 38%, transparent)`,
              }}
            />
            {tabs.map((t) => {
              ...
              return (
                <Link
                  key={t.to}
                  to={t.to}
                  className="relative flex flex-1 flex-col items-center gap-0.5 rounded-2xl py-2 text-xs font-medium"
                >
                  {active && (
                    <motion.span
                      layoutId="tab-pill"
                      className={`absolute inset-0 rounded-2xl shadow-glow ${cat.gradient}`}
                      transition={{ type: "spring", stiffness: 380, damping: 30 }}
                    />
                  )}
                  <Icon
                    className={`relative h-5 w-5 transition ${active ? "text-white" : "text-muted-foreground"}`}
                  />
                  <span
                    className={`relative transition ${active ? "text-white" : "text-muted-foreground"}`}
                  >
                    {t.label}
                  </span>
                </Link>
            ...
            >
              ...
              <div className="p-4 space-y-2 overflow-y-auto">
                {[
                  ...
                  >
                    <span
                      className={`grid h-10 w-10 place-items-center rounded-xl text-white shadow-soft ${m.grad}`}
                    >
