...
import { Ruler, Weight, Calendar, Flame, Target, Activity, Save, RotateCcw, Sparkles } from "lucide-react";
...
function CalculatorPage() {
  ...
  const { bmr, tdee, idealLo, idealHi, bfPct, bsa, whr } = useMemo(() => {
    ...
    let bfPct = 0;
    if (gender === "male") {
      bfPct = 86.01 * Math.log10(Math.max(1, waist - neck)) - 70.041 * Math.log10(Math.max(1, heightFinal)) + 36.76;
    } else {
      bfPct = 163.205 * Math.log10(Math.max(1, waist + hip - neck)) - 97.684 * Math.log10(Math.max(1, heightFinal)) - 78.387;
    }
    // Mosteller body surface area
    const bsa = Math.sqrt((heightFinal * weightFinal) / 3600);
    const whr = waist / hip;
    return { bmr: Math.round(bmr), tdee: Math.round(bmr * mult), idealLo, idealHi, bfPct: +bfPct.toFixed(1), bsa: +bsa.toFixed(2), whr: +whr.toFixed(2) };
  ...
  const save = () => {
    const entry = { id: crypto.randomUUID(), date: new Date().toISOString(), weight: +weightFinal.toFixed(1), height: Math.round(heightFinal), age, gender, bmi, category: cat.label };
    add(entry); set(entry);
    update({ weight: +weightFinal.toFixed(1), height: Math.round(heightFinal), age, gender, activityLevel: activity });
    navigate({ to: "/result" });
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="relative text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="font-display text-3xl font-black mt-0.5">{bmi.toFixed(1)} <span className="text-sm font-medium opacity-85">BMI</span></h2>
              <p className="text-xs opacity-90 mt-0.5">{cat.label} · ideal {idealLo}–{idealHi} kg</p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-center">
              <div className="rounded-xl bg-white/20 backdrop-blur px-2 py-1.5"><p className="font-display font-black text-sm">{bmr}</p><p className="text-[8px] uppercase opacity-80">BMR</p></div>
              <div className="rounded-xl bg-white/20 backdrop-blur px-2 py-1.5"><p className="font-display font-black text-sm">{tdee}</p><p className="text-[8px] uppercase opacity-80">TDEE</p></div>
            </div>
          </div>
      ...
      {/* Unit toggle */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-2 gap-1 rounded-2xl bg-secondary p-1">
        {(["metric", "imperial"] as Unit[]).map((u) => (
          <button key={u} onClick={() => setUnit(u)}
            ...
            {unit === u && <motion.span layoutId="unit-pill" className={`absolute inset-0 rounded-xl ${cat.gradient}`} />}
            <span className="relative">{u === "metric" ? "Metric (kg / cm)" : "Imperial (lb / ft)"}</span>
          </button>
        ))}
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="font-display font-bold flex items-center gap-2"><Ruler className="h-4 w-4" /> Height</p>
        {unit === "metric" ? (
        ...
        <p className="font-display font-bold flex items-center gap-2 pt-1"><Weight className="h-4 w-4" /> Weight</p>
        {unit === "metric"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="font-display font-bold flex items-center gap-2"><Calendar className="h-4 w-4" /> Age</p>
            <SliderRow value={age} min={1} max={120} unit="yr" onChange={setAge} />
          </div>
          <div>
            <p className="font-display font-bold flex items-center gap-2"><Sparkles className="h-4 w-4" /> Gender</p>
            <div className="mt-2 grid grid-cols-2 gap-2">
              {(["male", "female"] as const).map((g) => (
                <button key={g} onClick={() => setGender(g)}
                  className={`rounded-xl py-2 text-xs font-bold capitalize transition ${gender === g ? `${cat.gradient} text-white shadow-soft` : "bg-secondary"}`}>{g}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div>
          <p className="font-display font-bold flex items-center gap-2"><Activity className="h-4 w-4" /> Activity</p>
          <div className="mt-2 grid grid-cols-3 gap-2">
            {(["low", "moderate", "high"] as const).map((a) => (
              <button key={a} onClick={() => setActivity(a)}
                className={`rounded-xl py-2 text-xs font-bold capitalize transition ${activity === a ? `${cat.gradient} text-white shadow-soft` : "bg-secondary"}`}>{a}</button>
            ))}
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        {gender === "female" && <SliderRow label="Hip" value={hip} min={60} max={170} unit="cm" onChange={setHip} />}
      </motion.div>

      {/* Results grid */}
      <div className="grid grid-cols-2 gap-3">
        <ResultTile icon={Flame} label="BMR" value={`${bmr}`} sub="kcal/day" grad="bg-gradient-fire" />
        <ResultTile icon={Target} label="TDEE" value={`${tdee}`} sub="kcal/day" grad="bg-gradient-brand" />
        <ResultTile icon={Activity} label="Body fat" value={`${bfPct}%`} sub="US Navy" grad="bg-gradient-ocean" />
        <ResultTile icon={Sparkles} label="BSA" value={`${bsa}`} sub="m² (Mosteller)" grad="bg-gradient-mint" />
        <ResultTile icon={Ruler} label="Ideal" value={`${idealLo}-${idealHi}`} sub="kg range" grad={cat.gradient} />
        <ResultTile icon={Weight} label="WHR" value={`${whr}`} sub={whr < 0.9 ? "low risk" : whr < 1 ? "moderate" : "high"} grad="bg-gradient-sunset" />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="text-[11px] text-muted-foreground mt-3 leading-snug">
          Calculated from TDEE using the Mifflin–St Jeor equation. Adjust portions to your goal in the Goals screen.
        </p>
      </motion.div>

      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => { setWeight(profile.weight); setHeightCm(profile.height); setAge(profile.age); }}
  ...
  return (
    <div>
      <div className="flex justify-between text-xs">
        ...
        <span className="font-display font-black text-base">{value}<span className="text-[10px] text-muted-foreground ml-1">{unit}</span></span>
      </div>
