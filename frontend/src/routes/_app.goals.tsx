...
import { Target, TrendingDown, TrendingUp, Minus, Flag, Calendar, Flame, Droplet, Footprints, Apple, ArrowRight, Trophy } from "lucide-react";
import { bmiCategory, calcBMI, useActivities, useHistory, useProfile, suggestGoalWeight } from "@/lib/bmi-store";
...
function GoalsPage() {
  ...
  const suggested = suggestGoalWeight({ height: profile.height, goal: profile.goal, weight: profile.weight });
  ...
  const { weeks, calDelta, milestones, pct } = useMemo(() => {
    ...
    const pct = span === 0 ? 100 : Math.min(100, Math.max(0, Math.round((progressed / span) * 100)));
    const steps = 4;
    ...
    <div className="space-y-5">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-end justify-between mt-1">
          <div>
            ...
            <p className="text-sm opacity-90 mt-1">{diff > 0 ? `Lose ${diff} kg` : diff < 0 ? `Gain ${Math.abs(diff)} kg` : "You're at target 🎉"}</p>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {([
            { k: "lose", icon: TrendingDown, label: "Lose", grad: "bg-gradient-sunset", desc: "0.5kg/wk" },
            { k: "maintain", icon: Minus, label: "Maintain", grad: "bg-gradient-mint", desc: "Hold steady" },
            { k: "gain", icon: TrendingUp, label: "Gain", grad: "bg-gradient-ocean", desc: "Lean mass" },
          ] as const).map((g) => {
            const active = profile.goal === g.k;
            return (
              <motion.button key={g.k} whileTap={{ scale: 0.95 }}
                onClick={() => {
                  const t = suggestGoalWeight({ height: profile.height, goal: g.k, weight: profile.weight });
                ...
                <p className={`text-[10px] mt-0.5 ${active ? "opacity-90" : "text-muted-foreground"}`}>{g.desc}</p>
              </motion.button>
            );
          })}
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <DailyGoal icon={Flame} label="Calories" v={`${2000 + (diff > 0 ? -calDelta : calDelta)}`} sub="kcal/day" grad="bg-gradient-fire" />
          <DailyGoal icon={Droplet} label="Water" v="2L" sub="hydration" grad="bg-gradient-ocean" />
          <DailyGoal icon={Footprints} label="Steps" v={diff > 0 ? "10 000" : "7 000"} sub="movement" grad="bg-gradient-mint" />
          <DailyGoal icon={Apple} label="Protein" v={`${Math.round(profile.weight * 1.6)}g`} sub="1.6 g/kg" grad={cat.gradient} />
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="font-display font-bold flex items-center gap-2 mb-3"><Trophy className="h-4 w-4 text-amber-500" /> Milestones</p>
        <ol className="relative border-l-2 border-dashed border-border pl-4 space-y-3">
          {milestones.map((m, i) => (
            <li key={i} className="relative">
              <span className={`absolute -left-[22px] top-0.5 grid h-5 w-5 place-items-center rounded-full text-white text-[10px] font-black ${m.reached ? cat.gradient : "bg-secondary text-muted-foreground"}`}>{i + 1}</span>
              <div className="flex items-center justify-between">
                <p className="font-semibold text-sm">{m.label}</p>
                <p className={`text-xs font-bold ${m.reached ? "text-emerald-500" : "text-muted-foreground"}`}>{m.weightAt} kg</p>
              </div>
              <p className="text-[11px] text-muted-foreground">{m.reached ? "✓ Reached" : "Upcoming"}</p>
            </li>
          ))}
        </ol>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="font-display font-bold flex items-center gap-2"><Flag className="h-4 w-4" /> Suggested target</p>
        <p className="text-sm text-muted-foreground">Healthy ideal for your height: <span className="font-bold text-foreground">{suggested} kg</span></p>
        <button onClick={() => update({ targetWeight: suggested })}
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-3xl bg-card p-5 shadow-soft">
        <div className="flex items-center justify-between">
          <p className="font-display font-bold flex items-center gap-2"><Target className="h-4 w-4" /> Fine-tune target</p>
          <p className="font-display font-black text-lg">{profile.targetWeight} kg</p>
        </div>
        <input type="range" min={30} max={200} value={profile.targetWeight}
          onChange={(e) => update({ targetWeight: +e.target.value })}
          className="w-full mt-3 accent-primary" />
        <div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>30 kg</span><span>200 kg</span></div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link to="/tips" className={`flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-semibold text-white shadow-glow active:scale-95 ${cat.gradient}`}>
      ...
      <div className="rounded-3xl bg-card p-5 shadow-soft text-sm text-muted-foreground">
        ...
        <p>{acts.length} logged activities · {acts.reduce((s, a) => s + a.minutes, 0)} total minutes</p>
      </div>
    </div>
  );
}
