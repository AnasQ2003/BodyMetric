      ...
      <div className="rounded-3xl bg-card shadow-soft overflow-hidden divide-y">
        ...
        <div className="flex items-center gap-3 p-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">Tint avatars &amp; gradients</p>
          </div>
          <input
            type="range"
            min={0}
        ...
        <Link to="/goals" className="flex w-full items-center gap-3 p-4 active:bg-secondary/50">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">Adjust target weight</p>
          </div>
        ...
        <RowBtn
          ...
          label="Data &amp; storage"
          desc={`${history.items.length} logs · ${acts.items.length} activities`}
          onClick={() => setSheet("data")}
        />
        <RowBtn
          icon={Star}
          grad="bg-gradient-sunset"
          label="Rate BMI Pulse"
...
function PrivacySheet({ onClose }: { onClose: () => void }) {
  const items = [
    ...
    {
      Icon: Database,
      t: "No accounts required",
      d: "Use BMI Pulse without signing up to anything.",
    },
  ...
  return (
    <>
      ...
      <div className="p-5 space-y-3">
        ...
        <button
          onClick={onClose}
          className="w-full rounded-2xl bg-gradient-fire py-3 font-display font-bold text-white shadow-glow"
        >
          Got it
        </button>
      </div>
    ...
    <>
      ...
      <div className="p-5 space-y-3 text-sm">
        <p className="text-muted-foreground leading-relaxed">
          A beautifully animated BMI tracker with goals, history, charts, tips and personalized
          reports — all running locally on your device.
        </p>
...
function DataSheet({ onClose, onClear, history, acts }: any) {
  ...
  return (
    <>
      <SheetHead
        title="Data &amp; storage"
        subtitle="Everything stored locally"
        grad="bg-gradient-mint"
        Icon={Database}
        onClose={onClose}
      />
      <div className="p-5 space-y-3 text-sm">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-2xl bg-secondary p-3">
            <p className="font-display font-black text-xl">{history}</p>
            <p className="text-[10px] uppercase text-muted-foreground tracking-wider">BMI logs</p>
          </div>
          <div className="rounded-2xl bg-secondary p-3">
            <p className="font-display font-black text-xl">{acts}</p>
        ...
        {!confirm ? (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-destructive/10 py-3 font-semibold text-destructive"
          >
          ...
          <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-3 space-y-2">
            <p className="text-xs text-destructive font-semibold">
              This permanently deletes all logs &amp; activities.
            </p>
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => setConfirm(false)}
                className="rounded-xl bg-secondary py-2 text-xs font-semibold"
              >
                Cancel
              </button>
              <button
                onClick={onClear}
                className="rounded-xl bg-destructive py-2 text-xs font-bold text-white"
              >
                Yes, delete
              </button>
            </div>
...
function RateSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      <SheetHead
        title="Rate the app"
        subtitle="Your feedback shapes BMI Pulse"
        grad="bg-gradient-sunset"
        Icon={Star}
        onClose={onClose}
      />
      <div className="p-5 space-y-4">
        <div className="flex justify-center gap-2">
          {[1, 2, 3, 4, 5].map((n) => (
            ...
            >
              <Star
                className={`h-9 w-9 transition ${n <= stars ? "fill-amber-400 text-amber-400 drop-shadow" : "text-muted-foreground"}`}
              />
            </motion.button>
          ))}
        </div>
        <p className="text-center text-xs text-muted-foreground">
          {stars > 0 ? `Thanks for the ${stars}★ — keep tracking!` : "Tap to rate"}
        </p>
        <button
          disabled={!stars}
          onClick={onClose}
...
function Toggle({ icon: Icon, grad, label, desc, value, onChange }: any) {
  return (
    <div className="flex items-center gap-3 p-4">
      <div
        className={`grid h-11 w-11 place-items-center rounded-2xl text-white shadow-soft ${grad}`}
      >
...
function RowBtn({ icon: Icon, grad, label, desc, onClick }: any) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-3 p-4 text-left active:bg-secondary/50 transition"
    >
      <div
        className={`grid h-11 w-11 place-items-center rounded-2xl text-white shadow-soft ${grad}`}
      >
