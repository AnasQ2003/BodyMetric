...
import { ArrowRight, Share2, RotateCcw, Target, Flame, Apple, Dumbbell, Ruler, Activity, Heart, AlertCircle, CheckCircle2, TrendingDown, TrendingUp } from "lucide-react";
...
function ResultPage() {
  ...
  useEffect(() => {
    if (last) { setWeight(last.weight); setHeight(last.height); }
  }, [last]);
  ...
  const { calories, ideal, weightDelta, healthScore, weeksToGoal } = useMemo(() => {
    ...
    const mult = profile.activityLevel === "high" ? 1.55 : profile.activityLevel === "low" ? 1.2 : 1.375;
    ...
    return { calories: tdee, ideal: { lo: idealLo, hi: idealHi }, weightDelta: delta, healthScore: score, weeksToGoal };
  }, [weight, height, profile]);

  const compare = items.slice(0, 6).reverse().map((e) => ({
  ...
  if (!last) {
    return (
      <div className="rounded-3xl bg-card p-6 text-center shadow-soft">
        ...
        <Link to="/home" className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-gradient-brand px-4 py-2 text-white font-medium">
    ...
    <div className="space-y-5">
      ...
      <motion.div
        ...
        <div className="mt-3 flex items-end gap-3">
          <motion.p className="font-display text-7xl font-black leading-none">{animated.toFixed(1)}</motion.p>
          <span className="mb-2 rounded-full bg-white/25 px-3 py-1 text-xs font-bold backdrop-blur">{cat.label}</span>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          {[
            ...
            { l: weightDelta > 0 ? "To lose" : weightDelta < 0 ? "To gain" : "On target", v: `${Math.abs(weightDelta)}`, s: "kg" },
          ].map((s) => (
            <div key={s.l} className="rounded-xl bg-white/15 backdrop-blur p-2 text-center">
              <p className="font-display font-black text-base">{s.v}<span className="text-[10px] opacity-80 ml-0.5">{s.s}</span></p>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <div className="flex justify-between text-xs"><span className="text-muted-foreground">Weight</span><span className="font-bold">{weight} kg</span></div>
          <input type="range" min={20} max={250} value={weight} onChange={(e) => setWeight(+e.target.value)} className="w-full accent-primary" />
        </div>
        <div>
          <div className="flex justify-between text-xs"><span className="text-muted-foreground">Height</span><span className="font-bold">{height} cm</span></div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: Flame, label: "Calories", value: calories, sub: "kcal/day", grad: "bg-gradient-fire" },
          { icon: Target, label: "Target", value: `${profile.targetWeight}kg`, sub: `${weeksToGoal}w to go`, grad: "bg-gradient-mint" },
          { icon: Apple, label: "Goal", value: profile.goal, sub: profile.activityLevel, grad: "bg-gradient-ocean" },
          { icon: Ruler, label: "Range", value: `${ideal.lo}-${ideal.hi}`, sub: "kg ideal", grad: cat.gradient },
          { icon: Activity, label: "BMR", value: Math.round(calories / (profile.activityLevel === "high" ? 1.55 : profile.activityLevel === "low" ? 1.2 : 1.375)), sub: "rest kcal", grad: "bg-gradient-sunset" },
          { icon: Heart, label: "Risk", value: cat.risk, sub: cat.label, grad: "bg-gradient-brand" },
        ].map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <p className="font-display text-base font-black mt-1.5 capitalize leading-tight">{s.value}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-3xl bg-card p-5 shadow-soft">
        <p className="font-display font-bold flex items-center gap-2 mb-3"><Dumbbell className="h-4 w-4" /> Your action plan</p>
        <p className="text-sm text-muted-foreground leading-relaxed mb-3">{cat.message}</p>
        <ol className="space-y-2.5">
          {advice.map((a, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-white text-xs font-black ${cat.gradient}`}>{i + 1}</span>
              <div>
        ...
        <Link to="/tips" className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-primary">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="font-display font-bold flex items-center gap-2">{cat.risk === "safe" ? <CheckCircle2 className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />} Risk profile</p>
      ...
      {compare.length > 1 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="font-display font-bold mb-2 flex items-center gap-2">
            {weightDelta < 0 ? <TrendingUp className="h-4 w-4 text-emerald-500" /> : <TrendingDown className="h-4 w-4 text-rose-500" />}
            Recent comparison
          </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/home" className="flex items-center justify-center gap-2 rounded-2xl bg-secondary py-3 text-sm font-semibold active:scale-95">
          <RotateCcw className="h-4 w-4" /> Recalc
        </Link>
        <button className={`flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-semibold text-white shadow-glow active:scale-95 ${cat.gradient}`}>
...
function adviceFor(key: string) {
  const map: Record<string, { title: string; body: string }[]> = {
    under: [
      ...
      { title: "Strength train 3×/week", body: "Compound lifts build healthy mass faster than cardio." },
    ...
    obese: [
      ...
      { title: "Build sustainable swaps", body: "Replace, don't restrict — focus on 80% adherence." },
    ],
  };
  return map[key];
}
...
function riskCopy(key: string) {
  switch (key) {
    case "under": return "Higher chance of nutrient deficiency, low bone density and immune issues.";
    case "healthy": return "Lowest baseline risk for metabolic disease. Keep building the habits.";
    case "over": return "Elevated risk for type-2 diabetes, hypertension and joint stress.";
    default: return "Significantly higher cardiometabolic risk. Small, daily wins compound — start today.";
  }
}
