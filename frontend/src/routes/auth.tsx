...
const features = [
  {
    icon: Activity,
    title: "Live BMI engine",
    desc: "Real‑time scoring with risk meter",
    grad: "from-fuchsia-400 to-pink-500",
  },
  {
    icon: TrendingUp,
    title: "Smart insights",
    desc: "Track weight & BMI side by side",
    grad: "from-sky-400 to-cyan-500",
  },
  {
    icon: Apple,
    title: "Personalized tips",
    desc: "Tailored to your category",
  ...
  return (
    <div className="relative h-full overflow-hidden bg-gradient-brand text-white">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className="absolute top-1/3 -right-10 h-72 w-72 rounded-full bg-fuchsia-400/30 blur-3xl animate-blob"
          style={{ animationDelay: "2s" }}
        />
        <div
          className="absolute bottom-0 right-0 h-96 w-96 rounded-full bg-cyan-300/25 blur-3xl animate-blob"
      ...
      <div className="relative mx-auto flex h-full max-w-md flex-col px-6 pt-12 pb-10">
        ...
        <motion.h1
          ...
          className="mt-5 text-center font-display text-3xl font-black leading-tight"
        >
          Welcome to{" "}
        ...
        <motion.p
          ...
          className="mt-2 text-center text-sm text-white/85 max-w-xs mx-auto"
        >
          Your private, beautifully animated health companion.
        </motion.p>
        ...
        >
          {features.map((f, i) => (
            ...
            >
              <div
                className={`grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br ${f.grad} text-white shadow-soft`}
              >
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[11px] uppercase tracking-widest text-white/80 font-bold">
            Live · {samples.length}k tracking now
          </p>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div
            className={`grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${s.grad} font-display font-black text-sm shadow-soft`}
          >
            {s.name[0]}
          </div>
      ...
      <div className="mt-2.5 flex gap-1">
        {samples.map((_, i) => (
          <span
            key={i}
            className={`h-1 flex-1 rounded-full transition ${i === idx ? "bg-white" : "bg-white/25"}`}
          />
        ))}
      </div>
    </motion.div>
...
function InfoModal({ kind, onClose }: { kind: "terms" | "privacy"; onClose: () => void }) {
  ...
  const sections = isTerms
    ? [
        {
          t: "Acceptance",
          b: "By using BMI Pulse, you agree to use it as a personal wellness companion — not as medical advice.",
        },
        {
          t: "Your account",
          b: "Keep your credentials safe. You're responsible for activity on your account.",
        },
        {
          t: "Health disclaimer",
          b: "BMI is a general indicator, not a diagnosis. Consult a professional for medical guidance.",
        },
        {
          t: "Data ownership",
          b: "Your measurements belong to you. Export or delete them any time from Settings.",
        },
      ]
    : [
        {
          t: "Local-first",
          b: "All measurements live on your device. We don't ship them anywhere by default.",
        },
        { t: "No tracking", b: "No third-party analytics, no ads, no fingerprinting. Period." },
        {
          t: "Encryption",
          b: "Account sync (when enabled) uses end-to-end encryption with your key.",
        },
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`relative p-5 text-white ${isTerms ? "bg-gradient-sunset" : "bg-gradient-ocean"}`}
        >
          <button
            onClick={onClose}
