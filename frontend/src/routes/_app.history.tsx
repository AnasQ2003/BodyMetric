...
function HistoryPage() {
  ...
  const goalSeries = useMemo(
    () => ordered.map((e, i) => ({ x: bmiSeries[i]?.x || "", y: 22 })),
    [ordered, bmiSeries],
  );
  ...
  const weeklyAvg = useMemo(() => {
    ...
    return Object.entries(buckets)
      .slice(-6)
      .map(([x, ys]) => ({ x, y: +(ys.reduce((a, b) => a + b, 0) / ys.length).toFixed(1) }));
  }, [filtered]);
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs opacity-90 mt-1">
              Avg BMI {stats?.avg ?? "—"} · Δ {stats?.delta ?? "0"}
            </p>
          </div>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Filter className="h-3.5 w-3.5" /> Range
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {(["all", "healthy", "under", "over", "obese"] as Filt[]).map((f) => (
            <button
              key={f}
              onClick={() => setFilt(f)}
              className={`rounded-full px-3 py-1 text-[11px] font-bold capitalize transition ${filt === f ? "bg-gradient-brand text-white shadow-soft" : "bg-secondary text-muted-foreground"}`}
            >
              {f}
            </button>
          ))}
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex gap-2 text-[10px] font-bold">
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-primary" /> BMI
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full" style={{ background: "var(--brand-2)" }} />{" "}
                kg
              </span>
              <span className="flex items-center gap-1">
      ...
      >
        <p className="font-display font-bold mb-3 flex items-center gap-2">
          <Activity className="h-4 w-4" /> Category distribution
        </p>
      ...
      {filtered.length > 0 &&
        (() => {
          ...
          const stability = Math.max(
            0,
            1 -
              (Math.max(...filtered.map((i) => i.bmi)) - Math.min(...filtered.map((i) => i.bmi))) /
                8,
          );
          const target = profile.targetWeight
            ? Math.max(0, 1 - Math.abs(avgW - profile.targetWeight) / 20)
            : 0.5;
          const trend =
            filtered.length > 1
          ...
          return (
            ...
            >
              <p className="font-display font-bold mb-1 flex items-center gap-2">
                <Sparkles className="h-4 w-4" /> Health profile
              </p>
              <p className="text-xs text-muted-foreground mb-2">5-axis comparison vs ideal</p>
              <div className="grid place-items-center">
                <RadarChart
                  axes={["Healthy", "Stable", "On-Goal", "Steady", "Logged"]}
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <button
            ...
            className="grid h-8 w-8 place-items-center rounded-xl bg-secondary"
          >
            ‹
          </button>
      ...
      {filtered.length === 0 ? (
        ...
        <>
          <button onClick={clear} className="text-xs text-destructive font-semibold ml-auto block">
            Clear all
          </button>
            ...
            <AnimatePresence initial={false}>
              {filtered.map((it, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`grid h-14 w-14 place-items-center rounded-2xl text-white ${cat.gradient}`}
                    >
...
function PdfModal({
  ...
  onClose: () => void;
  onGenerate: (from: Date, to: Date, label: string) => void;
}) {
  const today = new Date();
  const iso = (d: Date) => d.toISOString().slice(0, 10);
  const [preset, setPreset] = useState<"week" | "month" | "quarter" | "year" | "all" | "custom">(
    "month",
  );
  const [from, setFrom] = useState(
    iso(new Date(today.getFullYear(), today.getMonth() - 1, today.getDate())),
  );
      ...
      >
        ...
        <div className="p-5 space-y-3">
          ...
          <p className="text-[10px] text-center text-muted-foreground">
            Includes profile, summary, BMI &amp; weight charts, distribution and full table.
          </p>
        </div>
      </motion.div>
    </motion.div>
