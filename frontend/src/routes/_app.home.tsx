...
import { ChevronUp, ChevronDown, Calculator, Target, Flame, TrendingUp, AlertTriangle, ShieldCheck, Droplet, Footprints, Moon, Activity, Sparkles, ArrowRight } from "lucide-react";
import { bmiCategory, calcBMI, useHistory, useLastResult, useProfile, useActivities } from "@/lib/bmi-store";
...
function HomePage() {
  ...
  const trend = useMemo(() => {
    ...
    return arr.map((e) => ({ x: new Date(e.date).toLocaleDateString(undefined, { month: "short", day: "numeric" }), y: e.bmi }));
  }, [rangedItems]);

  const weightTrend = useMemo(() => {
    const arr = [...rangedItems].slice(0, 30).reverse();
    return arr.map((e) => ({ x: new Date(e.date).toLocaleDateString(undefined, { month: "short", day: "numeric" }), y: e.weight }));
  }, [rangedItems]);

  const weekStart = Date.now() - 7 * 86400e3;
  const weekMin = acts.filter((a) => new Date(a.date).getTime() > weekStart).reduce((s, a) => s + a.minutes, 0);
  const weekCal = acts.filter((a) => new Date(a.date).getTime() > weekStart).reduce((s, a) => s + a.calories, 0);
  ...
  const handleCalc = () => {
    const entry = { id: crypto.randomUUID(), date: new Date().toISOString(), weight, height, age, gender: profile.gender, bmi, category: cat.label };
    add(entry); set(entry); update({ weight, height, age });
    navigate({ to: "/result" });
  };

  const streak = useMemo(() => {
    let s = 0; let cursor = new Date(); cursor.setHours(0, 0, 0, 0);
    const dates = new Set(items.map((i) => new Date(i.date).toDateString()));
    while (dates.has(cursor.toDateString())) { s++; cursor.setDate(cursor.getDate() - 1); }
    return s;
  }, [items]);
  ...
  return (
    <div className="space-y-5">
      ...
      <motion.div
        ...
        <div className="absolute -left-12 bottom-0 h-36 w-36 rounded-full bg-white/10 blur-3xl animate-blob" style={{ animationDelay: "2s" }} />
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-widest opacity-85 flex items-center gap-1.5">
              ...
              Live pulse · {new Date().toLocaleDateString(undefined, { weekday: "short", day: "numeric", month: "short" })}
            </p>
            <p className="font-display text-base mt-1 opacity-95">Hey <span className="font-bold">{profile.name.split(" ")[0] || "there"}</span> 👋</p>
            <div className="flex items-baseline gap-2 mt-1">
              <motion.h2 key={bmi.toFixed(1)} initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                transition={{ type: "spring", stiffness: 240, damping: 16 }}
                className="font-display text-4xl font-black drop-shadow">{bmi.toFixed(1)}</motion.h2>
          ...
          <motion.div key={cat.key} initial={{ scale: 0, rotate: -90 }} animate={{ scale: 1, rotate: 0 }}
            ...
            <motion.span className="absolute inset-0 rounded-2xl ring-2 ring-white/50"
        ...
        <div className="relative mt-4 grid grid-cols-4 gap-2 text-center">
          {[
            ...
            <motion.div key={s.k} whileTap={{ scale: 0.92 }}
              className="rounded-xl bg-white/15 backdrop-blur p-2">
              <s.i className="h-3.5 w-3.5 mx-auto opacity-90" />
              <p className="font-display font-black text-[13px] mt-1 leading-none">{s.v}<span className="text-[9px] font-medium opacity-80 ml-0.5">{(s as any).sub || ""}</span></p>
        ...
        <div className="relative mt-3">
          ...
          <div className="mt-1 h-2 w-full overflow-hidden rounded-full bg-white/20">
            <motion.div initial={{ width: 0 }}
              animate={{ width: `${Math.min(100, Math.max(0, 100 - Math.abs(profile.weight - profile.targetWeight) * 5))}%` }}
              transition={{ duration: 1 }} className="h-full rounded-full bg-white shadow-glow" />
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold text-white ${cat.gradient}`}>{cat.label}</span>
        </div>
        <div className="grid grid-cols-[1fr_auto] gap-2 items-center">
          <div className="grid place-items-center"><RiskMeter value={bmi} color={cat.color} /></div>
          <DonutChart value={Math.min(100, Math.max(0, 100 - Math.abs(profile.weight - profile.targetWeight) * 5))} label="Goal" sub={`${profile.targetWeight}kg`} color={cat.color} />
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <DailyTile icon={Droplet} label="Water" value="1.4L" sub="of 2L" grad="bg-gradient-ocean" pct={70} />
        <DailyTile icon={Footprints} label="Steps" value="6.2k" sub="of 8k" grad="bg-gradient-mint" pct={77} />
        <DailyTile icon={Moon} label="Sleep" value="7h 20m" sub="of 8h" grad="bg-gradient-sunset" pct={91} />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/calculator" className="text-xs font-bold text-primary flex items-center gap-1">Full version <ArrowRight className="h-3 w-3" /></Link>
        </div>
        <div className="flex items-baseline justify-between">
          <p className="text-sm text-muted-foreground font-medium">Height</p>
          <p className="font-display"><span className="text-3xl font-black">{height}</span><span className="text-sm font-medium text-muted-foreground ml-1">cm</span></p>
        </div>
        <input type="range" min={120} max={220} value={height} onChange={(e) => setHeight(+e.target.value)} className="w-full accent-primary" />

        <div className="grid grid-cols-2 gap-3 pt-2">
          <Stepper label="Weight" unit="kg" value={weight} onChange={setWeight} min={20} max={250} />
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <Link to="/history" className="text-xs font-bold text-primary">See all →</Link>
        </div>
        <div className="grid grid-cols-4 gap-1 bg-secondary rounded-xl p-1 mb-3">
          {(["7d", "30d", "90d", "all"] as const).map((r) => (
            <button key={r} onClick={() => setChartRange(r)}
              ...
              {chartRange === r && <motion.span layoutId="home-chart-pill" className={`absolute inset-0 rounded-lg ${cat.gradient}`} />}
        ...
        {trend.length >= 1 ? (
          ...
          <p className="text-center text-xs text-muted-foreground py-8">No entries in this range. Try a longer window.</p>
        )}
      </motion.div>
      ...
      {dayBars.some((d) => d.y > 0) && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-muted-foreground">{weekCal} kcal · {weekMin}m</span>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="font-display text-lg font-bold mt-1 leading-snug">"Small habits, when stacked daily, become your transformation."</p>
        <Link to="/tips" className="mt-3 inline-flex items-center gap-1 rounded-full bg-white/20 px-3 py-1 text-xs font-bold backdrop-blur">
          Explore tips <ArrowRight className="h-3 w-3" />
        </Link>
      </motion.div>

      {/* CTA */}
      <motion.button whileTap={{ scale: 0.96 }} onClick={handleCalc}
...
function DailyTile({ icon: Icon, label, value, sub, grad, pct }: any) {
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-[10px] uppercase tracking-wider opacity-90">{label} · {sub}</p>
      <div className="mt-2 h-1.5 w-full rounded-full bg-white/30 overflow-hidden">
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.9 }} className="h-full bg-white" />
      </div>
    </motion.div>
  );
}
...
function Stepper({ label, unit, value, onChange, min, max }: any) {
  return (
    <div className="rounded-2xl bg-secondary p-3">
      ...
      <p className="font-display mt-1"><span className="text-2xl font-black">{value}</span><span className="text-xs text-muted-foreground ml-1">{unit}</span></p>
      <div className="mt-2 flex justify-between gap-2">
        <button onClick={() => onChange(Math.max(min, value - 1))}
          className="grid h-9 w-9 place-items-center rounded-xl bg-card active:scale-95"><ChevronDown className="h-4 w-4" /></button>
        <button onClick={() => onChange(Math.min(max, value + 1))}
          className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-brand text-white shadow-glow active:scale-95"><ChevronUp className="h-4 w-4" /></button>
      </div>
    </div>
  );
}
